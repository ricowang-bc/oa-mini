<template>
    <u-cell-group>
		...
		<u-cell-item title="请假时间" @click="showCalendar=true" :value="`共 ${ request.dayCount } 天`"></u-cell-item>
        <view style="padding:20rpx 30rpx" class="u-flex u-row-between" v-if="duration.length>0">
            <text style="font-size: 24rpx;">{{durationDates.join(', ')}}</text>
        </view>
	</u-cell-group>
    ...
    <u-calendar v-model="showCalendar" mode="mulit" @change="onChangeDays" :safe-area-inset-bottom="true" max-date="2050-12-31" />
...
const onChangeDays = (e:any) => {
    let dateArray = e.result.map((x:any)=>new Date(x)).sort((a:Date,b:Date)=>{return a.getTime()-b.getTime();});
    request.value.dayCount = dateArray.length;
