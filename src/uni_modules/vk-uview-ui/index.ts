...
// import mpShare from './libs/mixin/mpShare'
// 引入关于是否mixin集成小程序分享的配置
// import wxshare from './libs/mixin/mpShare.js'
// 全局挂载引入http相关请求拦截插件
import http from './libs/request/index'

function wranning(str:any) {
...
// 对象转字符串，或者字符串转对象
import addStyle from './libs/function/addStyle'
...
const install = (Vue:any) => {
  ...
  // #ifndef VUE3
	...
	Vue.filter('timeFormat', (timestamp:any, format:any) => {
		return timeFormat(timestamp, format)
	})
	Vue.filter("date", (timestamp: any, format: any) => {
        return timeFormat(timestamp, format)
    })
	// 将多久以前的方法，注入到全局过滤器
	Vue.filter("timeFrom", (timestamp: any, format: any) => {
